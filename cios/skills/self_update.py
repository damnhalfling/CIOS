"""Skill: Self-Update — check for new versions and update CIOS.
...
def check_update(use_cache: bool = True) -> UpdateInfo:
    ...
    try:
        req = urllib.request.Request(
            _GITHUB_API,
            headers={"Accept": "application/vnd.github.v3+json", "User-Agent": f"CIOS/{current}"},
        )
        ...
        for asset in data.get("assets", []):
            name = asset.get("name", "")
            if name.endswith("_amd64.deb") and "cios" in name:
...
def download_and_install(info: UpdateInfo) -> tuple[list[str], bool, str]:
    ...
    # Download
    deb_path = Path(f"/tmp/cios_{info.latest_version}_amd64.deb")
    ...
    try:
        ...
        return (steps, False, "Falha no download. Verifique sua conexão.")

    # Validate filename
    if not re.match(r"^cios_[\d.]+_amd64\.deb$", deb_path.name):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return (
                ...
                "Reinicie o CIOS para usar a nova versão.",
            )
        else:
        ...
        return (steps, False, f"Erro na instalação. O pacote está em {deb_path}")
...
def check_update_summary() -> tuple[list[str], str]:
    ...
    if info.has_update:
        ...
        summary = f"CIOS v{info.current_version} — já está na versão mais recente."

    return steps, summary


def boot_update_check() -> str | None:
    ...
    try:
        ...
        if info.has_update:
            return (
                f"Nova versão disponível: v{info.latest_version}. "
                'Diga "atualizar cios" para instalar.'
            )
    except Exception:
        pass
    return None
