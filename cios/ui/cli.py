...
def run_ui() -> None:
    ...
    history_file = CIOS_HOME / "history.txt"
    ...
    while True:
        try:
            user_input = session.prompt(
                HTML("<ansicyan><b>cios</b></ansicyan> <ansiwhite>›</ansiwhite> "),
            ).strip()
        except (EOFError, KeyboardInterrupt):
            console.print("\n[dim]CIOS detaching. System continues.[/dim]")
            break
        ...
        if user_input.lower() in ("exit", "quit", "q"):
            console.print("[dim]CIOS detaching. System continues.[/dim]")
            break
