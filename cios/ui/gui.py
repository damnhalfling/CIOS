...
class ThreadPanel:
    ...
    def _render_entry(self, thread, is_active: bool = False) -> tk.Frame:
        ...
        tk.Label(
            top_row, text=icon, font=self._fonts["small"],
            fg=icon_color, bg=bg,
        ...
        tk.Label(
            top_row, text=summary, font=self._fonts["list_t"],
            fg=FG if is_active else FG_SEC, bg=bg,
            anchor="w",
        ...
        tk.Label(
            top_row, text=timestamp_str, font=self._fonts["small"],
            fg=FG_DIM, bg=bg,
        ).pack(side="right")
    ...
    def _render_turns(self, parent: tk.Frame, thread, bg: str) -> None:
        ...
        if not turns:
            tk.Label(
                parent, text="  Sem detalhes",
                font=self._fonts["small"], fg=FG_DIM, bg=bg,
                anchor="w",
        ...
        for turn in turns:
            ...
            if user_text:
                tk.Label(
                    turn_frame, text=f"→ {user_text}",
                    font=self._fonts["small"], fg=FG_SEC, bg=bg,
...
class CIOSApp:
    ...
    def __init__(self) -> None:
        ...
        self._active_gallery: Optional[GalleryComponent] = None
        # Voice
    ...
    def _build_page_home(self) -> None:
        ...
        self._pending_indicator = tk.Label(
            self._thread_panel_area, text="⏳ Aguardando resposta...",
            font=self._f["small"], fg=ACCENT_LT, bg=BG, anchor="w")

        # Timeout warning indicator (hidden by default)
        self._timeout_warning = tk.Label(
            self._thread_panel_area, text="⚠ Tempo quase esgotado",
            font=self._f["small"], fg=WARNING, bg=BG, anchor="w")

        # Thread panel header
        thread_header = tk.Frame(self._thread_panel_area, bg=BG)
        thread_header.pack(fill="x", pady=(SP_TIGHT, SP_MICRO))
        tk.Label(thread_header, text="🕐 Conversas", font=self._f["small"],
                 fg=FG_DIM, bg=BG).pack(side="left")
        self._show_all_btn = tk.Label(
            thread_header, text="ver tudo", font=self._f["small"],
        ...
        self._thread_panel = ThreadPanel(
            self._thread_panel_area, self._bridge, self._f)
    ...
    def _build_right_panel(self) -> None:
        ...
        # ── System Status ──
        tk.Label(top, text="📈 Status do Sistema", font=self._f["sec"],
        ...
        env_text = (f"Host: {platform.node()}\n"
                    ...
                    f"Interface: Tkinter")
        tk.Label(top, text=env_text, font=self._f["small"], fg=FG_DIM,
        ...
        # ── Suggestions ──
        tk.Label(top, text="💡 Sugestões", font=self._f["sec"],
                 fg=FG_SEC, bg=BG_PANEL, anchor="w").pack(
            fill="x", padx=SP_DEFAULT, pady=(SP_DEFAULT, SP_COMPACT))
        ...
        tk.Frame(rp, bg=BORDER, height=1).pack(
            side="bottom", fill="x", padx=SP_DEFAULT, pady=0)
    ...
    def _refresh_intelligence_ui(self) -> None:
        ...
        if not intelligence.is_logged_in:
            ...
            tk.Label(card, text="⚡ CIOS Intelligence",
                     font=self._f["sec"], fg=ACCENT_LT, bg=BG_CARD,
                     anchor="w").pack(fill="x")

            tk.Label(card, text="Resumos, traduções e textos\ngerados por IA avançada.",
                     font=self._f["small"], fg=FG_SEC, bg=BG_CARD,
                     justify="left", anchor="w").pack(fill="x", pady=(SP_TIGHT, 0))

            tk.Label(card, text="✓ 5 consultas/dia grátis\n✓ Sem cartão de crédito",
                     font=self._f["small"], fg=SUCCESS, bg=BG_CARD,
                     justify="left", anchor="w").pack(fill="x", pady=(SP_TIGHT, 0))

            btn = tk.Frame(card, bg=ACCENT, padx=SP_DEFAULT, pady=SP_TIGHT)
            btn.pack(fill="x", pady=(SP_DEFAULT, 0))
            tk.Label(btn, text="🔑 Entrar com Google",
            ...
            tk.Label(info_row, text="👤", font=("Helvetica", 16),
                     fg=FG_SEC, bg=BG_CARD).pack(side="left", padx=(0, SP_TIGHT))

            name_frame = tk.Frame(info_row, bg=BG_CARD)
            name_frame.pack(side="left", fill="x")
            tk.Label(name_frame, text=user.name or user.email.split("@")[0],
                     font=self._f["metric_v"], fg=FG, bg=BG_CARD,
                     anchor="w").pack(fill="x")

            plan_text = f"Plano: {usage.plan.capitalize()}"
            plan_color = ACCENT_LT if usage.plan != "free" else FG_SEC
            tk.Label(name_frame, text=plan_text,
                     font=self._f["small"], fg=plan_color, bg=BG_CARD,
                     anchor="w").pack(fill="x")
            ...
            if used >= limit:
                # Limit reached
                tk.Label(usage_frame, text="⚠️ Limite diário atingido",
                         font=self._f["small"], fg=WARNING, bg=BG_CARD,
                         anchor="w").pack(fill="x")
                tk.Label(usage_frame, text="Renova amanhã às 00:00",
                         font=self._f["small"], fg=FG_DIM, bg=BG_CARD,
                         anchor="w").pack(fill="x")
            else:
                tk.Label(usage_frame, text=f"Hoje: {used}/{limit} consultas",
                         font=self._f["small"], fg=FG_SEC, bg=BG_CARD,
                         anchor="w").pack(fill="x")
            ...
            if usage.plan == "free":
                ...
                tk.Label(upgrade_btn, text="⚡ Upgrade → Starter",
                         font=self._f["small"], fg="#fff", bg=ACCENT_DK,
                         cursor="hand2").pack()
    ...
    def _update_pending_indicators(self) -> None:
        ...
        if has_pending:
            if not self._pending_indicator.winfo_manager():
                self._pending_indicator.pack(fill="x", pady=(SP_TIGHT, 0),
                                             before=self._thread_panel.widget)
        else:
        ...
        if has_pending and time_remaining is not None and time_remaining < 30:
            ...
            self._timeout_warning.configure(
                text=f"⚠ Tempo quase esgotado ({remaining_int}s)")
            if not self._timeout_warning.winfo_manager():
                self._timeout_warning.pack(fill="x", pady=(SP_TIGHT, 0),
                                           before=self._thread_panel.widget)
        else:
