...
def handle_intent_media(intent: Intent, executor: Executor, memory: Memory) -> PlanResult:
    ...
    from cios.skills.media_player import scan_media, play_media, stop_playback, GallerySignal
    ...
    app = (find_app("vlc") or find_app("mpv") or
           find_app("browser") or find_app("firefox"))

    if not app:
        from cios.core.humanizer import _LANG
        msg = ("Nenhum player encontrado. Instale com: instalar mpv"
               if _LANG == "pt"
               else "No player found. Install with: install mpv")
        return PlanResult(
            plan_steps=["Looking for player"], results=[],
            outcome="failure", summary=msg)

    steps, ok, err = resilient_call(
