...
def handle_system_health(intent: Intent, executor: Executor, memory: Memory) -> PlanResult:
    ...
    return PlanResult(
        ...
        else ("failure" if report.status == "critical" else "recovered"),
...
def handle_power(intent: Intent, executor: Executor, memory: Memory) -> PlanResult:
    ...
    if action == "battery_status":
        ...
        if pct < 15:
            ...
            summary += "\n⚠ Battery getting low"
        return PlanResult(
            plan_steps=["Checking battery"], results=[], outcome="success", summary=summary
        )
    ...
    if action == "brightness_status":
        ...
        if level < 0:
            return PlanResult(
                plan_steps=["Checking brightness"],
                results=[],
                outcome="failure",
        ...
        return PlanResult(
            ...
            results=[],
            outcome="success",
            summary=f"Brightness: {level}%",
        )

    if action == "brightness_up":
        delta = intent.params.get("delta", 10)
        steps, ok, msg = resilient_call(power_skill.change_brightness, delta, skill="power")
        return PlanResult(
    ...
    if action == "brightness_set":
        ...
        steps, ok, msg = resilient_call(power_skill.set_brightness, level, skill="power")
        return PlanResult(
            plan_steps=steps, results=[], outcome="success" if ok else "failure", summary=msg
        )

    if action == "power_saving":
        steps, ok, msg = resilient_call(power_skill.enable_power_saving, skill="power")
        return PlanResult(
            plan_steps=steps, results=[], outcome="success" if ok else "failure", summary=msg
        )
