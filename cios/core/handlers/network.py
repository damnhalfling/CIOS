...
def handle_network(intent: Intent, executor: Executor, memory: Memory) -> PlanResult:
    ...
    if action == "status":
        ...
        if wifi.connected:
            ...
            return PlanResult(
                plan_steps=["Checking Wi-Fi"], results=[], outcome="success", summary=summary
            )
        return PlanResult(
    ...
    if action == "disconnect":
        steps, ok, msg = resilient_call(network_skill.disconnect, skill="network")
        return PlanResult(
    ...
    if action == "connect":
        ...
        steps, ok, msg = resilient_call(network_skill.connect, ssid, password, skill="network")
        return PlanResult(
            plan_steps=steps,
            results=[],
