...
def check_system_health() -> HealthReport:
    ...
    if mem.percent > 90:
        summary.append(f"⚠ Memory is almost full ({mem.percent:.0f}% — {mem_used_gb:.1f} / {mem_total_gb:.1f} GB)")
        warnings += 1
    elif mem.percent > 75:
        summary.append(f"Memory is getting full ({mem.percent:.0f}% — {mem_used_gb:.1f} / {mem_total_gb:.1f} GB)")
    else:
        summary.append(f"Memory is fine ({mem.percent:.0f}% — {mem_used_gb:.1f} / {mem_total_gb:.1f} GB)")
    ...
    if disk.percent > 90:
        summary.append(f"⚠ Storage is almost full ({disk_free_gb:.1f} GB free of {disk_total_gb:.0f} GB)")
        suggestions.append('Say "free space" to find what\'s using your disk')
        warnings += 1
    elif disk.percent > 80:
        summary.append(f"Storage is getting full ({disk_free_gb:.1f} GB free of {disk_total_gb:.0f} GB)")
    else:
    ...
    for proc in psutil.process_iter(["pid", "name", "cpu_percent", "memory_percent", "memory_info"]):
        try:
            ...
            if mem_pct > 1.0 or cpu_pct > 5.0:
                ...
                if name in ("systemd", "kworker", "Xwayland", "Xorg",
                            "pipewire", "pulseaudio", "dbus-daemon"):
                    continue
                top_procs.append({
                    "name": name,
                    "pid": info["pid"],
    ...
    if top_procs:
        ...
        for p in top_procs:
            ...
            summary.append(
                f"  {p['name']} — {mem_str} memory, "
                f"{p['cpu']:.0f}% processor"
            )
