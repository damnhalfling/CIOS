"""Daemon mode — Unix socket server for Harmoni.

Runs Harmoni as a background daemon, accepting commands via a Unix socket.
This allows other processes (hotkey, top bar, CLI) to communicate with
the running Harmoni instance without starting a new process each time.
...
Usage:
    ...
    echo '{"command": "status"}' | socat - UNIX-CONNECT:/tmp/harmoni.sock
...
from harmoni.core.config import ensure_dirs, HARMONI_HOME

logger = logging.getLogger(__name__)

SOCKET_PATH = "/tmp/harmoni.sock"
PID_FILE = HARMONI_HOME / "daemon.pid"
_MAX_MSG_SIZE = 65536


class HarmoniDaemon:
    """Unix socket daemon for Harmoni."""

    def __init__(self) -> None:
        self._bridge: Optional[HarmoniBridge] = None
    ...
    def start(self) -> None:
        ...
        if self._is_running():
            print(f"Harmoni daemon already running (PID file: {PID_FILE})")
            sys.exit(1)
        ...
        self._running = True
        logger.info("Harmoni daemon started on %s (PID %d)", SOCKET_PATH, os.getpid())
        print(f"Harmoni daemon running on {SOCKET_PATH}")
