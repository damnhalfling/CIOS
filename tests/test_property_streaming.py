"""Property-based tests for streaming execution callbacks.
...
# COMMAND_EXEC intents with "run echo <word>" always produce exactly one plan step.
...
class TestStreamingCallbacksProperty:
    ...
    def test_streaming_invokes_on_step_at_least_once(self, command: str):
        ...
        **Validates: Requirements 3.3**
        ...
        bridge = _make_bridge()
        try:
            steps_received: list[tuple[str, int, int]] = []

            def on_step(text: str, index: int, total: int) -> None:
            ...
            # There must be at least one plan step callback
            assert len(plan_steps) >= 1, (
                f"on_step was called only with phase labels "
                f"{[s[0] for s in steps_received]}, "
