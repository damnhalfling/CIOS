...
def _make_mock_executor(server_pid: int = 42) -> MagicMock:
    ...
    executor.run.return_value = ExecResult(
        command="mock", returncode=0, stdout="ok", stderr="", duration=0.0,
    )
    mock_proc = MagicMock()
    ...
    executor.kill_by_port.return_value = ExecResult(
        command="kill", returncode=0, stdout="", stderr="", duration=0.0,
    )
    return executor
...
class TestDevStartWorkflowProperty:
    ...
    def test_dev_start_plan_correctness(
        ...
        **Validates: Requirements 1.1, 1.3, 1.5**
        ...
        if deps_installed:
            install_steps = [s for s in plan if "install" in s.lower() or "dependencies" in s.lower()]
        ...
        if port_occupied:
            ...
            port_indices = [i for i, s in enumerate(plan) if "port" in s.lower() and ("kill" in s.lower() or "in use" in s.lower())]
